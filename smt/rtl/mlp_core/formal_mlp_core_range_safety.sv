module formal_mlp_core_range_safety;
  ...
  always @(posedge clk) begin
    ...
    if (step >= 7'd2) begin
      ...
      if (formal_do_mac_hidden) begin
        assert (formal_state == MAC_HIDDEN);
        assert (formal_input_idx < 4'd4);
        assert (formal_hidden_input_case_hit);
        assert (formal_hidden_weight_case_hit);
      end

      if (formal_state == MAC_HIDDEN) begin
        assert (formal_input_idx <= 4'd4);
      end

      if ((formal_state == MAC_HIDDEN) && (formal_input_idx == 4'd4)) begin
        assert (!formal_do_mac_hidden);
        assert (!formal_hidden_input_case_hit);
        assert (!formal_hidden_weight_case_hit);
        assert (formal_mac_a == 16'sd0);
      end

      if (formal_do_mac_output) begin
        assert (formal_state == MAC_OUTPUT);
        assert (formal_input_idx < 4'd8);
        assert (formal_hidden_idx == 4'd0);
        assert (formal_output_hidden_case_hit);
        assert (formal_output_weight_case_hit);
      end

      if (formal_state == MAC_OUTPUT) begin
        assert (formal_input_idx <= 4'd8);
        assert (formal_hidden_idx == 4'd0);
      end

      if ((formal_state == MAC_OUTPUT) && (formal_input_idx == 4'd8)) begin
