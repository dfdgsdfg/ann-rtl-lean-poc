...
**Range argument.** The same two-model framework from [`from-ann-to-proven-hardware.md` §5](from-ann-to-proven-hardware.md) applies here at the bitvector level. The wide-sum checks confirm that for all inputs in `[-128, 127]⁴`, the reachable 32-bit values stay within signed range, so wraparound is declared but never active for this frozen instance. Change the weights, and this argument has to be re-established.
...
The miter is compositional across widths. Each layer operates at a different bit width — 16-bit products, 32-bit accumulators, 24-bit output products — and each layer checks that the contract-side and RTL-side views agree at that width. If the hidden products agree, and the accumulation of those products agrees, then the equivalence propagates through the forward pass to the final `out_bit`.
...
**Two arithmetic views.** The arithmetic overlap between Lean and SMT has a precise structure. Both layers prove that wrapping does not change the result, but they do so in different arithmetic models. Lean works with unbounded integers (the bridge theorem `mlpFixed_eq_mlpSpec` shows fixed-point equals unbounded). SMT works with fixed-width bitvectors (the wide-sum checks show 32-bit agrees with a wider reference computation). For these frozen weights, both routes establish the same practical fact: wrapping never changes the reachable result on the checked input domain.
